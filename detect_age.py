...
# construct the arguments can parsed from command line
...
# define age groups which our age detector will predict
AGE_BUCKETS = ["(0-2)", "(4-6)", "(8-12)", "(15-20)", "(25-32)", "(38-43)", "(48-53)", "(60-100)"]

# load face detector pre-trained model from disk
print("[INFO] loading face detector model...")
prototxtPath = os.path.sep.join([args["face"], "deploy.prototxt"])
weightsPath = os.path.sep.join([args["face"], "res10_300x300_ssd_iter_140000.caffemodel"])
faceNet = cv2.dnn.readNet(prototxtPath, weightsPath)
# load age detector pre-trained model from disk
...
for i in range(0, detections.shape[2]):
	...
	if confidence > args["confidence"]:
		...
		# extract the the face ROI and then construct a blob from *only* the face ROI
